...
def incremental_train(outfile, class_map, map_reverse, num_iters, train_data, test_data):
    with open(outfile, 'w') as file:
        ...
        for s in range(0, num_iters, num_classes):
            ...
            #     print (f"Train loader batch size: {images.size()}", file=file)


            # 这用的clean_test 不知道什么作用，先进行修改
            # clean_test_loader = torch.utils.data.DataLoader(clean_test(test_data), batch_size=batch_size,
            #                                     shuffle=True, num_workers=0)
            test_set = test_data[s]
            clean_test_loader = torch.utils.data.DataLoader(test_set, batch_size=batch_size,
                                                shuffle=True, num_workers=0)

            for images, labels in clean_test_loader:
                print (f"clean test loader batch size: {images.size()}", file=file)
            # 上述修改
            ...
            print ('\n', file=file)
            print ('\n')
            if(s % model_batch == 0 and (s == clientNum*0 or s == clientNum*7 or s == clientNum*5 or s == clientNum*2)): torch.save(model, f"./Models/Incremental/{model_batch}.pth")
    return model
...
def re_incremental_train(start, start_model, outfile, class_map, map_reverse, num_iters, train_data, test_data):
    with open(outfile, 'a') as file:
        ...
        for s in range(start, num_iters, num_classes):
            ...
            print (f'Time : {t2 - t1}', file=file)
            print ('\n', file=file)
            if(s % model_batch == 0): torch.save(model, f"./Models/Incremental/new_{model_batch}.pth")
    return model
